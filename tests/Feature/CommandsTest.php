...
class CommandsTest extends TestCase
{
    ...
    /**
     * Under `Cross-Origin-Embedder-Policy: require-corp` a cross-origin asset loaded
     * without `crossorigin` is refused, and the browser turns that into a silently
     * missing asset. The doctor is the place that catches it before deploy.
     *
     * Only for an application that asked for the policy. It is off by default — see
     * `pwax.security.cross_origin_embedder_policy` — so warning about it unprompted would
     * be warning about a rule nobody is enforcing.
     */
    ...
    public function test_the_install_command_publishes_the_config(): void
    {
        ...
        // Removed again by `TestCase::tearDown()`, which cleans up after every test rather
        // than only the ones that reach their last line — see the note there for what a
        // published config left inside `vendor/` does to every run after it.
    ...
    public function test_the_doctor_command_warns_about_a_browser_css_engine(): void
    {
        $this->manifest_clean();

        config()->set('pwax.head.image', 'https://example.test/og.png');
        // Ships a compiler to every visitor and builds the stylesheet by reading the DOM,
        // so nothing is styled until the download, the parse and the scan have all finished
        // — on every cold load, offline included.
    ...
    public function test_the_doctor_command_recognises_a_self_hosted_css_engine(): void
    {
        $this->manifest_clean();

        config()->set('pwax.head.image', 'https://example.test/og.png');
        // Serving the engine from your own origin fixes the privacy and the offline half of
        // the problem and none of the cost, so the warning is about the engine rather than
        // about where it came from.
        config()->set('pwax.scripts', [['src' => '/js/tailwindcss/browser.js', 'head' => true]]);

        $this->artisan('pwax:doctor')
            ->expectsOutputToContain("Tailwind's browser build")
            ->assertSuccessful();
    }

    public function test_the_doctor_command_is_quiet_about_an_ordinary_script(): void
    {
        $this->manifest_clean();
        ...
        $this->artisan('pwax:doctor')
            ->doesntExpectOutputToContain('builds its stylesheet')
            ->assertSuccessful();
    }
