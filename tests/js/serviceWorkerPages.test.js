...
describe('page payloads offline', () => {
     ...
     * once without for the document. Fetching only the first leaves every route the
     * visitor has not personally opened with no HTML — offline and on a cold start they
     * get the shell and a spinner while the runtime fetches a payload already on disk.
     */
...
describe('a server that is failing rather than absent', () => {
    ...
    it('serves the stored page when the origin cannot be reached', async () => {
        ...
        // resolves — and the visitor is shown an error while a copy of the page sits on
        // the device unread. 503 is what `php artisan down` answers with.
        const caches = new FakeCaches();
        const current = manifest();
