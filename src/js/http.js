 ...
 * `Pwax::LOCATION_HEADER` in `src/Pwax.php`. They must agree; `RuntimeContractTest`
 * asserts that.
 */
...
export function createHttp(config) {
     ...
     *
     * Returned fresh each time: a `Headers` instance is mutable, so a single shared one
     * would let anything that adds a header to one request add it to all of them.
     */
