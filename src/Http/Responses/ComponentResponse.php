 ...
 *
 * Both branches set `Vary`. Without it any CDN or shared proxy may cache one
 * representation and serve it for the other, which shows raw JSON to a browser or breaks
 * navigation in the runtime.
 */
class ComponentResponse implements Responsable
{
    ...
    private function json(Component $component): JsonResponse
    {
        ...
        // just left.
        //
        // An empty head is safe to send. Every value here is already resolved against the
        // application's defaults, so a page that declares nothing carries the application's
        // description rather than the previous page's — and clearing the canonical, the
        // description and the managed tags is the correct answer for a page that genuinely
        // has none.
        ...
        // An asset, not a document: no framing or permissions headers, but `nosniff` very
        // much applies — this is JSON, and a browser talked into treating it as something
        // else is the whole reason that header exists.
        return app(SecurityHeaders::class)->apply($response, document: false);
    }
    ...
    private function shell(Component $component, Request $request): Response
    {
        ...
        // The shell carries this session's CSRF token, so it is nobody else's to reuse.
        ...
        // The same set the offline shell carries. Before this the two disagreed: a
        // navigation the server answered was unhardened and the identical navigation the
        // service worker answered from the shell was not, so the application's own
        // behaviour changed the moment it went offline.
        return app(SecurityHeaders::class)->apply($response, document: true);
    }
}
