 ...
 *      per call, and module records are never collected — one dead module per navigation,
 ...
 * unique per call, and ES module records are keyed by URL and never garbage collected, so
 * compiling on every navigation would leave one dead module behind each time. With the
 * hash as the key, returning to a page reuses the module already compiled.
 *
 * @param {string} source
 * @param {string} hash
 * @returns {Promise<any>}
