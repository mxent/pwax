 ...
 * back whatever HTML sits at the far end. In 1.x that meant an `auth` middleware
 * redirect surfaced in the browser as an opaque JSON parse failure labelled "Network
 * Error" — the `redirect` key the client already knew how to handle was never produced
 * by anything on the server.
 *
...
{
    public function __construct(private readonly Pwax $pwax) {}
