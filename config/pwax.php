...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | The `<PwaxJson>` component renders a JSON document — a tree of components
    | and props, rather than a template — against the catalog declared here. It
    | is how a screen composed on the server, or generated by a model, reaches
    | the browser without anyone shipping markup: a document can only name a
    | component this catalog lists, and can only pass props it declares.
    |
    |     <PwaxJson :json="doc" />
    |
    | enabled     Serves, precaches and loads the renderer. Off means the
    |             component still exists, renders nothing, and says so — which
    |             is a better error than an unresolved component tag.
    | components  The catalog, keyed by the name a document uses. A value is
    |             either a component reference on its own, or an array adding
    |             the description and prop types a generator needs.
    | actions     Handlers a document can dispatch with `on`, on top of the
    |             built-in `navigate`, `submit` and `reload`.
    |
    | A component reference reads exactly like `vue` above: an
    | "@pwaxImport('view.name')" string, a "module:view.name" string, or a
    | dotted path to look up on `window`. None of the three is ever evaluated.
    |
    | A catalog component is an ordinary Pwax component. One rule is worth
    | knowing: it receives its children through a single default `<slot />`,
    | because a document has no way to address a named one. What a document
    | *can* bind is whatever the component declares in `emits` — that is where
    | the events in `on` come from, and nothing here has to repeat them.
    |
    | The renderer is a second bundle, `dist/pwax-json.js`. It carries
    | @json-render/vue and its dependencies and is around 82 kB gzipped against
    | the runtime's 9.7 kB, so it is fetched by the first `<PwaxJson>` that
    | renders and never on a page that has none.
    |
    |     'components' => [
    ...
    |         ],
    |
    |         // A library component already on the page. `events` is the escape
    |         // hatch for one whose `emits` cannot be read from its options.
    |         'Chart' => ['component' => 'MyCharts.Line', 'events' => ['point-click']],
    |     ],
    |
    | Prop types are `string`, `number`, `boolean`, `enum` (with `values`),
    | `array`, `object` and `any`. A component that declares no props accepts
    | whatever the document sends, which is the right default for a document you
    | wrote yourself and the wrong one for a document a model wrote.
    |
    */
