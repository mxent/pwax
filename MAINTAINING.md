...
| JS header constants match `Pwax::HEADER` / `LOCATION_HEADER` | `tests/Feature/RuntimeContractTest.php` |
| `types/pwax.d.ts` `Config` matches `Shell::runtimeConfig()` | `tests/Feature/RuntimeContractTest.php` |
| `sw.json`'s `pageHeaders` covers every field in `Pwax::VARY` | `tests/Feature/OfflinePagesTest.php` |
...
**Keep it equal to the Composer version.** The two are separate files and nothing enforces
the equality, so it has to be a habit. Let them drift and the one version number a
developer can read from a browser console names a release that does not exist. There is
one product and one number for it.

```bash
# 1. Bump the runtime version to the one you are about to tag, and rebuild so the banner
#    and __PWAX_VERSION__ agree with it.
...
- **`package.json` is `"private": true` and is never published to npm.** The version there
  exists only to stamp the bundle, and to answer `window.pwax.version` — which is why it
  has to match the tag rather than drift on its own schedule. The Composer package is the
  release; the git tag is what Packagist reads.
...
Dropping a Laravel major is a breaking change: it needs a major version and a changelog
entry under it.
...
you delete it, say so in the changelog.
