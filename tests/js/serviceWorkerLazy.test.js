 ...
 *
 * Keeping them in the precache — which is named for the build and deleted wholesale on the
 * next one — would have every deploy silently discard every image and font on the device
 * and re-download each on next use: the exact churn the delta install exists to prevent,
 * applied to the largest files in the application. These tests are the contract that says
 * it does not happen: survive a deploy, drop only what actually changed, and let
