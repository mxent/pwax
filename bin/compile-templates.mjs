...
for (const [hash, template] of Object.entries(input.templates || {})) {
    try {
        const { code } = compiler.compile(template, {
            ...
            // promise of `pwax:compile`: the same page, rendered faster, not a different one.
            // Precompiling is per component and an incomplete store falls back to compiling
            // in the browser, so the two have to agree element for element.
            hoistStatic: false,
            ...
            // development build. A render function that keeps a template's HTML comments does
            // not match one that drops them, and this has to agree with the production
            // runtime the browser is served.
            comments: false,
        });
