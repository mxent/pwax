...
class MiddlewareTest extends TestCase
{
     ...
     * and dies in `response.json()` — a parse failure where a sign-in belongs.
     */
    ...
    /**
     * The two responses this middleware creates are Pwax's own, so they carry what every
     * other Pwax response carries. Everything else in the `web` group is passed through
     * untouched — hardening an application's own routes is not this middleware's job.
     */
    public function test_a_translated_redirect_is_hardened_and_a_passed_through_response_is_not(): void
    {
        $translated = $this->get('/needs-auth', $this->componentHeaders());

        $translated->assertHeader('X-Content-Type-Options', 'nosniff');
        $translated->assertHeader('Referrer-Policy', 'no-referrer');
        // Not a document: framing and permissions mean nothing on a `fetch()` response.
        $translated->assertHeaderMissing('X-Frame-Options');

        $this->get('/plain')->assertHeaderMissing('Referrer-Policy');
    }
    ...
    /**
     * Components are Blade views that may call auth(). With no middleware on the
     * component routes the session is never started, and every one of them renders as a
     * guest.
     */
