...
describe('registerServiceWorker', () => {
    ...
    /*
     * Connectivity is not the worker's business. Wiring it up inside a successful
     * registration would put `pwax:online` / `pwax:offline` behind the one thing whose
     * absence makes them most needed: the worker is off by default, so most applications
     * would never receive either — and neither would one served over plain HTTP, where
     * registration cannot succeed. `boot()` calls it instead; the assertion is that it
     * does not need a registration.
     */
    it('reports connectivity changes without a service worker', () => {
        watchConnectivity();
    ...
    /*
     * `boot()` runs again on `window.pwax.start()`, and a second pair of listeners would
     * announce every change twice.
     */
    it('adds its listeners once however often it is called', () => {
        watchConnectivity();
        watchConnectivity();
        watchConnectivity();
