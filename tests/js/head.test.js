...
describe('applying a page head', () => {
    ...
    it('does nothing at all when there is no head to apply', () => {
        document.head.innerHTML = '<meta name="description" content="site wide">';

        // `undefined` is not "a page with no description" — it is a payload with no head in
        // it at all, which is what a response from before `head` existed looks like. There
        // is nothing to apply and nothing to clear.
        applyHead(undefined);
    ...
    /**
     * The server resolves the description before it sends it: the page's own, or
     * `head.description`, or `manifest.description`, whichever exists first. So the
     * application-wide value arrives in the payload of a page that declares none, and is
     * applied like any other — there is nothing for the client to preserve.
     */
    it('applies the application-wide description on a page that names none', () => {
        document.head.innerHTML = '<meta name="description" content="A blog post.">';

        applyHead({ title: 'Acme', description: 'The Acme application.' });
    ...
    /**
     * And when there is no application-wide value either — neither `head.description` nor
     * `manifest.description` is set by default — the correct document has no description at
     * all.
     *
     * This is the case that used to leak. The description was left alone whenever a page
     * declared none, on the reasoning that the shell's tag belonged to the application. In
     * an application that configured no description it does not: it belongs to whichever
     * page the visitor landed on, and every page after it inherited that page's summary.
     */
    it("drops the previous page's description when neither the page nor the app has one", () => {
        document.head.innerHTML = '<meta name="description" content="A blog post.">';
