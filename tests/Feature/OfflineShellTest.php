...
class OfflineShellTest extends TestCase
{
    ...
    /**
     * The shell is rendered with no page at all, so its head comes entirely from the
     * application's defaults. It still has to be a complete document: a `<title>`-less
     * page is what an installed app shows in its task switcher when it starts offline.
     */
    public function test_the_offline_shell_carries_the_application_head(): void
    {
        config()->set('pwax.manifest.name', 'Ledger');
        config()->set('pwax.head.description', 'Books that balance.');

        $body = (string) $this->get('/__pwax__/shell')->getContent();

        $this->assertStringContainsString('<title>Ledger</title>', $body);
        $this->assertStringContainsString('name="description" content="Books that balance."', $body);
    }
