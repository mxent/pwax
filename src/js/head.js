...
/**
 * The description, removed when there is none — the same rule the canonical link follows.
 *
 * `head.description` is already resolved server-side: it is the page's own, or
 * `head.description`, or `manifest.description`, whichever exists first. So an absent value
 * does not mean "this page said nothing"; it means the page said nothing *and neither did
 * the application*, and the correct document has no description meta at all.
 *
 * Returning early here instead — on the reasoning that the shell emits an application-wide
 * description a page with none should keep — holds only for an application that configured
 * one, and neither `head.description` nor `manifest.description` is set by default. Without
 * one, the tag left in the document belongs to whichever page the visitor landed on, and
 * every page after it inherits that page's description. A wrong answer outliving a missing
 * one is the exact failure this module exists to prevent.
 *
