...
async function boot() {
    ...
    // `pwax:online` / `pwax:offline`, for every application rather than only the ones that
    // registered a worker. These are `window` events about the network, and an app with no
    ...
    // Mounting replaces the spinner's markup, but a *published* shell may put the loading
    // semantics on the mount element rather than on the inner element the bundled one uses.
    // Left there they turn the application root into a live region for the rest of the
    // session: every reactive text change announced, and the whole app labelled "Loading".
    // Normalised here rather than trusted, because the mount element is the one part of the
    // document an application can rewrite and the runtime still has to own.
