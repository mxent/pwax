...
class PrecompileTest extends TestCase
{
    ...
    /**
     * `@vue/compiler-dom` is an optional peer dependency, so a contributor who has not run
     * `npm ci` should not see this fail. Skipping is honest; pretending is not.
     *
     * CI is the exception, and sets `PWAX_REQUIRE_NODE=1`: a suite that skips every test of
     * a feature reports the same green tick as one that runs them, which is how a broken
     * `pwax:compile` reaches a release.
     */
    private function requireCompiler(): void
    {
        ...
        if (! is_dir($root . '/node_modules/@vue/compiler-dom')) {
            if (filter_var(getenv('PWAX_REQUIRE_NODE'), FILTER_VALIDATE_BOOL)) {
                $this->fail('PWAX_REQUIRE_NODE is set but @vue/compiler-dom is not installed; run `npm ci`.');
            }
