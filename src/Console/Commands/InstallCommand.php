...
class InstallCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Named here because it is the one thing a fresh install does not have and cannot
        // discover. Registering a service worker on somebody's origin is not a decision a
        // package gets to make for them, so it stays opt-in — but an installable, offline
        // application is what people come to this package for, and finding out that the
        // switch exists should not require reading the config file to the end.
        $this->line('  4. Turn the service worker on for offline support and installability:');
        $this->line('     <fg=gray>\'service_worker\' => [\'enabled\' => true],  // config/pwax.php</>');
        $this->line('     <fg=gray>Add icons under \'manifest.icons\' — without a 192 and a 512,</>');
        $this->line('     <fg=gray>no browser offers to install the app.</>');
        $this->line('  5. Check your setup at any time with <info>php artisan pwax:doctor</info>.');
        $this->newLine();
