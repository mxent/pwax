 ...
 *
 * The parser is deliberately shallow — it is a block splitter, not an HTML parser —
 * but it is depth-aware for `<template>` and attribute-tolerant everywhere, which the
 * 1.x `strpos`/`strrpos` approach was not. In particular:
 *
 *   - `<template lang="html">` and `<script setup>` are matched, not skipped.
 *   - Nested `<template v-if>` / `<template #slot>` tags no longer truncate the root
...
class BlockExtractor
{
    ...
    public function externalStyles(string $contents): array
    {
        ...
        foreach ($this->matchAll('/<link\b[^>]*>/i', $contents) as $match) {
            ...
            // to inject, and 1.x swept all of them into the payload indiscriminately.
