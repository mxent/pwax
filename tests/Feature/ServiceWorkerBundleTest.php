...
class ServiceWorkerBundleTest extends TestCase
{
    ...
    public function test_an_applications_own_blade_worker_replaces_the_bundle(): void
    {
        config()->set('pwax.service_worker.blade', 'sw.replacement');

        $body = $this->worker();

        // Outright, not alongside: no preamble, no bundle, no extensions.
        $this->assertStringContainsString('// an application supplying its own worker', $body);
