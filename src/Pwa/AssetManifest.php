 ...
 * produces a new cache and an old client can never mix versions. Shipping a change needs
 * no version bump at all: `service_worker.version` is there only for the case where you
 * want to discard everything deliberately.
 *
...
class AssetManifest
{
     ...
     *
     * `network-only` by default: an undeclared URL is passed straight through and nothing
     * is stored. A caching catch-all would keep a copy of everything it saw — a one-off
     * PDF, a CSV export, a file under `/storage` — all of it counting against the origin's
     * quota, none of it ever asked for offline. The entry cap counts entries rather than
     * bytes, so sixty large ones is a very different amount of disk from sixty small ones.
     *
     * An application that wants those responses cached declares them, and then knows it has.
     */
    ...
    /**
     * A digest of everything the shell is built from, for when the shell cannot be rendered.
     *
     * Blind to the shell's own markup, which is why it is the fallback rather than the
     * primary — but it still changes when the things most likely to change do, and a coarse
     * hash is a far better failure mode than a manifest build that throws and takes
     * `sw.json` with it.
     */
     ...
     *
     * Read in one place because it is compared against as well as listed. Two call sites
     * with their own hardcoded defaults would drift, and a changed path would leave the
     * entry unhashed and re-downloaded on every deploy.
     */
