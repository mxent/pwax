 ...
 * JSON block; nothing here is interpolated into JavaScript. Assembling the runtime inside
 * a Blade file with `{!! !!}` would put `config/pwax.php` on the syntax path of the page —
 * one stray quote and the whole document stops parsing.
 */
class Shell
{
     ...
     *     the page paints unstyled and is restyled a moment later.
    ...
    private function configuredScripts(bool $head): array
    {
        ...
        foreach ((array) $this->config->get('pwax.scripts', []) as $script) {
            ...
            // `head` is a placement instruction rather than an attribute. Left in place,
            // `attributes()` would render it as a boolean attribute and every one of these
            // tags would carry a stray `head` in the markup.
            unset($tag['head']);
     ...
     *
     * The keys are `pwax.vue.*`, the same ones {@see runtimeConfig()} reads — and they
     * have to stay the same ones. Read a key nobody writes and `extensions()` is handed an
     * empty array, so the second of the two sources above quietly emits nothing. The
     * failure is invisible from the outside: a missing resource hint costs a round trip
     * and never an error.
     *
     * @return list<string>
     */
     ...
     *
     * `source`, not "strategy": the other four keys by that name in `config/pwax.php`
     * choose a caching behaviour, and this one chooses a hostname.
     */
